...
namespace SmokeQuit.SoapAPIServices.LocDPX.SoapServices
{
    ...
    public interface IChatsLocDpxSoapService
    {
        ...
        [OperationContract]
        Task<ChatsLocDpx> GetByIdAsync(int chatId); // ✅ Keep parameter name as 'chatId'
    ...
    public class ChatsLocDpxSoapService : IChatsLocDpxSoapService
    {
        ...
        public async Task<ChatsLocDpx> GetByIdAsync(int chatId) // ✅ Keep 'chatId' parameter name
        {
            try
            {
                Console.WriteLine($"SOAP GetByIdAsync called with chatId: {chatId}");

                if (chatId <= 0)
                {
                    Console.WriteLine("Invalid chatId: must be greater than 0");
                    return new ChatsLocDpx();
                }

                // ✅ Keep your original method name as intended
                var chat = await _serviceProviders.ChatsService.GetGetByIdAsync(chatId);

                if (chat == null)
                {
                    Console.WriteLine($"No chat found with ID: {chatId}");
                    return new ChatsLocDpx();
                }

                Console.WriteLine($"Found chat: ID={chat.ChatsLocDpxid}, Message={chat.Message}");
        ...
        public async Task<ChatsLocDpx> UpdateAsync(ChatsLocDpx chat)
        {
            try
            {
                ...
                if (result > 0)
                {
                    // Return updated chat using the same method name pattern
